...
def mlp_short():
    ...
    model.compile(
        loss=keras.losses.CategoricalCrossentropy(), optimizer=optimizer, metrics=["accuracy"]
    )
    return model
...
def cnn_ascad():
    ...
    # Block 1
    x = Conv1D(32, 32, activation='relu', padding='same', name='block1_conv1')(img_input) #8
    Dropout(0.1)
